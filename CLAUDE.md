...
- After merging a PR, refresh the committed snapshot **via its own `docs/...` branch + PR** — `master` is protected (8 required status checks), so a direct `git commit` + push to `master` is rejected. Steps: (1) move any local `build*/` and `_deps/` directories out of the tree first — graphify scans them and does **not** honor `.gitignore`, which inflates the graph with vendored deps; (2) `graphify update . --force` (the rebuild has fewer nodes, so `--force` is required); (3) `cp graphify-out/GRAPH_REPORT.md docs/graph-report.md`; (4) restore the build dirs; (5) commit on a `docs/graph-snapshot-*` branch, open a PR, and merge once CI is green.
