...
| `exporter.compression` | `WithCompression(c)` | `OTEL_EXPORTER_OTLP_COMPRESSION` | — | `off` | `off` or `gzip`. Controls **requests**: gzip request bodies with `content-encoding: gzip` (HTTP) or frame flag `0x01` with `grpc-encoding: gzip` (gRPC). Responses are independent — `accept-encoding` / `grpc-accept-encoding: gzip` is advertised whatever this is set to, and a compressed response is inflated under `limits.max_decompressed_bytes`. |
