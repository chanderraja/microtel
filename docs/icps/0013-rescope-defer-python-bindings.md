# ICP 0013: Retire M8, re-add Python bindings as M17 covering all three signals

**Status:** Draft
**Affected interfaces / docs:** `microtel-spec.md` §6.2, §13 milestone table (M8, M10, new M17), §13.4, §13.5; `microtel-roadmap.md` §4 (v1.0 theme). No interface headers, no `docs/interfaces.md`.
...
bindings as **M17** covering traces, metrics, and logs, and drop Python from the
v1.0 target set.
...
2. **§13 table — append M17:**

   | Milestone | Scope | Effort | Depends on | Parallel with |
   |---|---|---|---|---|
   | **M17 – Python bindings (all signals)** | nanobind layer over traces, metrics, and logs; `logging.Handler` bridge; wheels via `cibuildwheel` for manylinux_2_28; examples. `microtel.__capabilities__` reports `{"traces": True, "metrics": True, "logs": True}`. | 4 wk | M10; thread-local current-span slot | M15, M16 |
...
5. **§13.5 release gates:** replace "Python bindings (M8) are a v1.0 *target*
   ...
   with: Python is not part of the v1.0 target set and ships post-v1.0 as M17.
6. **§6.2:** add a one-line note that the example's `start_as_current_span` is
   the M17 target shape and requires the thread-local current-span slot, which
...
so `RuntimeContext` and `trace::Scope` resolve inside the shim. Whether it
precedes M17 is a separate scheduling decision needing its own ICP, covering the
build-time dependency on the opentelemetry-cpp API headers (CLAUDE.md rules
12–13) and the ABI-configuration constraint those headers impose.
