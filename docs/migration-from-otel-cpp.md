...
| `TraceState` | Does **not** round-trip. `FromHeader`/`ToHeader` are defined as of issue #188, but `microtel::TraceState` still carries no storage, so it cannot hold an entry and both directions produce the empty default. | `context_conversion.hpp`'s file comment | `otelcpp_context_conversion_test.cpp` |
