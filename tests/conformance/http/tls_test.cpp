...
//
// Transport security over OTLP/HTTP, against a real collector.
//
// `tests/integration/transport/http2_tls_connect_test.cpp` already proves the
// verification logic added by ICP 0022 against a hand-rolled OpenSSL server.
// This file asks the complementary question: does the same configuration reach
// the *collector*, whose TLS stack, ALPN behaviour and client-auth policy are
// Go's and not ours? A hand-rolled peer cannot answer that, because both sides
// of it are written by the same people who wrote the client.
//
// The four receivers the runner exposes make the posture a property of the
// endpoint rather than of a restart:
//
//   otlp/tls  (4328)  server cert only          → TlsCustomCa, SniOverride
//   otlp/mtls (4338)  server cert + client CA   → MutualTls, and the negative
...
// in particular is the regression guard for ICP 0022 — before that change,
// `ca_bundle` was loaded into a trust store that `SSL_VERIFY_NONE` never
// consulted, so this exact test would have connected happily to a server the
// configured CA does not vouch for.
//
...
constexpr const char* kTlsEndpointEnv = "MICROTEL_CONFORMANCE_HTTP_TLS_ENDPOINT";
constexpr const char* kMtlsEndpointEnv = "MICROTEL_CONFORMANCE_HTTP_MTLS_ENDPOINT";
...
constexpr const char* kScopeName = "microtel.conformance.http.tls";
...
/// @brief Points @p builder at @p endpoint over OTLP/HTTP with test timeouts.
///
...
microtel::SdkBuilder& ConfigureHttpBuilder(microtel::SdkBuilder& builder,
...
TEST(HttpTlsConformance, TlsCustomCa)
{
    std::string endpoint;
    ...
    auto result = ConfigureHttpBuilder(builder, endpoint)
...
TEST(HttpTlsConformance, MutualTls)
{
    std::string endpoint;
...
TEST(HttpTlsConformance, MutualTlsWithoutClientCertFails)
{
    std::string endpoint;
    if (!microtel::testing::ConformanceEnabled(kMtlsEndpointEnv, endpoint))
    {
    ...
    // error. That is TLS 1.3 doing what it does — the client finishes the
    // handshake without waiting for the server to accept its (absent)
    // certificate, so the server's alert lands on the next read, which is the
    // read of the server's SETTINGS frame. Either way the connection never
    // becomes usable, which is the claim under test; the message is not
    // asserted because it is an artefact of when the alert arrives.
    const auto connected = provider->Connect();
...
TEST(HttpTlsConformance, SniOverride)
{
...
TEST(HttpTlsConformance, UntrustedCaFails)
{
    std::string endpoint;
    if (!microtel::testing::ConformanceEnabled(kTlsEndpointEnv, endpoint))
    {
    ...
    // The ICP 0022 regression guard. Observed failure against the pinned
    // collector: "TLS certificate verification failed: unable to get local
    // issuer certificate". Before ICP 0022 this connected, because the trust
    // store was loaded and then never consulted.
    const auto connected = provider->Connect();
