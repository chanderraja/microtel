...
/// @brief Dumb mock for `microtel::internal::ILogEncoder`.
///
/// Returns a 1-byte-allocated empty `EncodedPayload` on every call. No protobuf
/// encoding happens. Counts calls atomically so the test thread can read after
/// `ForceFlush`.
class MockLogEncoder : public internal::ILogEncoder
{
public:
    std::atomic<int> encode_call_count{0};

    [[nodiscard]] internal::EncodedPayload Encode(
        const internal::LogBatchHandle& /*batch*/) override
    {
        ++encode_call_count;
